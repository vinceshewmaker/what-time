    ...
    <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingTop: 16 }}>
        <div style={{ maxWidth: 480, margin: '0 auto' }}>
          ...
          {tab === 'home' && (
            <DashboardScreen state={state} onGoIV={() => setTab('iv')} />
          )}
          {tab === 'iv' && (
            <IVScreen state={state} who={session.who} updateState={updateState} />
          )}
          {tab === 'log' && (
            <QuickLogScreen state={state} who={session.who} updateState={updateState} />
          )}
        </div>
      </div>
